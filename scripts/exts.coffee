...
if !String::toSourceCode
  String::toSourceCode = () ->
    lines = @split('\n')
    c = lines.length.toString().length
    nl = []
    if lines
      for i in [0..lines.length - 1]
        nl.push("{0}| {1}".format(Number(i + 1).toString().padLeft(c, ' '), lines[i]))
    return nl.join('\n')
...
class StringBuilderClass
  constructor: (text, startline) ->
    if type(text) is 'number'
      startline = text
      text = null
    @startline = (if startline? then startline else 0)
    @clear()
    @_id = makeId()
    if text?
      @append(text)
  ...
  error: (msg) ->
    if !msg
      ...
      err.loc = {line: @currentLine()}
      @_error = err

  append: (text) ->
    if text
      if type(text) is 'array'
        for d in text
          @append(d)
      else
        t = (if text.toString then text.toString() else text)
        @values.push(t)
    return @
  ...
  _apply: (start, del, end, fct, cb) ->
    if start and type(start) is 'function'
      fct = start
      start = null

    if del and type(del) is 'function'
      fct = del
      del = null

    if end and type(end) is 'function'
      fct = end
      end = null
    ...
    cb.apply(@, [start, del, end, fct]) if cb

    return @

  _block: (start, del, end, fct) ->

    buf = new StringBuilderClass(@currentLine())
    buf.append(start)
    fct.apply(buf, [buf]) if fct
    buf.append(end)
    l = buf.toString().split('\n')
    if buf._error
      @error(@buf.error())
      return @
      ...
      for i in [0..len - 1]
        ...
        if s
          ...
          if i + 1 < len - 1
            ...
            nx = ''
          if i == len - 1 or ts.length == 0 or ts[ts.length - 1] in ['{', '[', '(', ':', ',', ';'] or nx.length == 0 or nx[0] in ['}', ']', ')']
            @line(s)
          else
            @line(s + del)
  ...
  list: (start, del, end, fct) ->
    @_apply(start, del, end, fct, (start, del, end, fct) ->
      @_block(start, del, end, fct)
    )
    return @

  parens: (start, end, fct) ->
    @_apply(start, ',', end, fct, (start, del, end, fct) ->
      @_block(start + '(', del, ')' + end, fct)
    )
    return @

  parensBlock: (start, end, fct) ->
    @_apply(start, ',\n', end, fct, (start, del, end, fct) ->
      @_block(start + '(', del, ')' + end + '\n', fct)
    )
    return @
  ...
  arrayBlock: (start, end, fct) ->
    @_apply(start, ',\n', end, fct, (start, del, end, fct) ->
      @_block(start + '[', del, ']' + end + '\n', fct)
    )
    return @

#  statement: (start, end, fct) ->
#    @_apply(start, '\n', end, fct, (start, del, end, fct) ->
#      @_block(start, del, ';' + end + '\n', fct)
#    )
#    return @

  assign: (start, end, fct) ->
    @_apply(start, '\n', end, fct, (start, del, end, fct) ->
      @_block(start + ' = ', del, end, fct)
    )
    return @

  nodes: (node, codetype, user, componentType, recursive, args) ->
    if node.$data?.isModule
      r = node.getRoot()
    else
      r = node

    if recursive and type(recursive) is 'array'
      args = recursive
      recursive = false

    if componentType == '*'
      _nodes = r.children(recursive)
    else
      _nodes = r.childrenOfKind(componentType.split(','), recursive)

    if _nodes
      buf = new StringBuilderClass(@currentLine())
      for n in _nodes
        n.generateCode(buf, codetype, user, args)
        if buf._error
          @error(@buf.error())
          return @
      @append(buf.toString())
...
exp.StringBuilder = StringBuilderClass
