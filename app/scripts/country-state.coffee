...
angular.module('countrystate.services', [])
...
.controller('CountryStateCtrl', [
  ...
  ($scope, countries, states, $timeout) ->
    ...
    $scope.countryChange = (selectize) ->
      if selectize.items and selectize.items.length
        val = selectize.items[0]
        idx = $scope.countries.indexOf(val)
        if idx != -1 and idx != $scope.prevStateIdx
          ...
          if type(a) is 'string'
            ...
            if elem.length
              statesSelectize = elem.selectize()[0].selectize
              if a.length and !a[0].name and !a[0].value
                for i in [0..a.length - 1]
                  a[i] = { value: a[i], name: a[i] }
              ...
              if ok
                $timeout(->
                  statesSelectize.setOptionsValue(v)
                )
